...
function displayResults(responseJson, maxResults) {
    ...
    for (let i = 0; i < responseJson.data.length & i < maxResults; i++) {
        $('.list').append(`<li><h3><a href="${responseJson.data[i].url}">${responseJson.data[i].fullName}</a></h3>
...
function watchForm() {
    $('.js-form').on('submit', function() {
        ...
        const stateArr = $('#term').val().split(",");
        const maxResults = $('#max-results').val();
        const apiKey = 'Gmc8uW58zqLayjaA7sdUUdtMXKib9eBUX9bQl6Wb'
